                                 ...
                                 gpointer            get_content_size_data)
{
  ...
  int extra_width, extra_height, extra_size;
  ...
  extra_width = border.left + border.right + padding.left + padding.right;
  extra_height = border.top + border.bottom + padding.top + padding.bottom;

  if (for_size > -1)
    {
      if (orientation == GTK_ORIENTATION_HORIZONTAL)
        for_size -= extra_height;
      else
        for_size -= extra_width;
    }
  ...
  if (orientation == GTK_ORIENTATION_HORIZONTAL)
    extra_size = extra_width;
  else
    extra_size = extra_height;

  *minimum += extra_size;
  *natural += extra_size;

  if (minimum_baseline && *minimum_baseline > -1)
    *minimum_baseline += extra_size;
  if (natural_baseline && *natural_baseline > -1)
    *natural_baseline += extra_size;
}
